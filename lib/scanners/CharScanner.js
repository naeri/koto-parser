...
class CharScanner {
	constructor(buffer, options) {
		...
		if (this.options.sanitize && !this.options.sanitized) {
			let sanitizeOption = this.options.sanitize;

			if (sanitizeOption === true) {
				sanitizeOption = undefined;
			}

			this.buffer = sanitizeHtml(this.buffer, sanitizeOption);
			this.options.sanitized = true;
		}
